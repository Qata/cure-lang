...
Data flow (explore mode): `Generators.Term.typed_term/1` emits a
`%Challenge{kind: :typed_term}` → Runner banks it in `seeds.sexp`
(coverage-deduped) → the selected assay runs → `:ok` or an infection that
writes a `tmp/antigen/` report and appends an antibody to `corpus.sexp`.
Replay mode decodes stored records and re-runs assays statically — no
generation, no StreamData, exactly the Tier-A discipline.
...
payload: %{
  ...
  ctx: [core_type],  # telescope, outermost first (index 0 = most recent, kernel order)
...
- `SignatureMenu.env_of(:v1)` declares families, adds defs, certifies them,
...
| Goal `T` (whnf) | Check-mode intros | Infer-mode eliminations (available at every goal) |
|---|---|---|
| `Pi(A, B)` | `lam` (recurse at `B` under `Γ,A`) | var whose type meets `T`; INDIR; plain app; `fst`/`snd` of a Γ-var of Sigma type whose component meets `T`; `case` (§6.5) |
| `Sigma(A, B)` | `pair` (gen `a : A`, then `b : B[a]`) | (same elimination menu) |
| `data`/`Vec(i)` | constructor whose result index unifies (§6.3) | (same elimination menu) |
| `Type 0` | a menu type former | var; INDIR |

Infer-mode candidates produce a term with an *inferred* type; the generator
accepts one only if `Conv.conv_within?(inferred, T, …, @gen_fuel)` returns
`{:ok, true}` — the interleaved-checking rule. Fuel exhaustion or `false` at
...
to `S(j)` (then recurse with `j`). When `i` is a stuck neutral (e.g. a
variable), only infer-mode rules apply — exactly the situation dependent
matching lives in, and precisely the terms the assays should see.
...
Per explore run, over *banked* (post-dedup) challenges:
...
Floors are module attributes beside the fuel constants.
...
- **Support-set checks:** `Gen.support/1` on the rule-table frequency draws
  stays `:finite` where the design says it must (choice points inspectable).

## 10. Acceptance criteria

1. `mix antigen` explore over the three new assays completes a 500-candidate
   run: every infection minted an antibody + report, every non-duplicate
   candidate banked as a seed, health metrics printed.
