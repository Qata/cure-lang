...
- **A stated law is subtly false** → the assay infects a sound kernel (false positive) — the highest-severity failure mode. This is concentrated in §3a law 3 (commutation index arithmetic). Mitigation: laws 1–2 and 4 are unconditionally true; law 3 is lifted from prior art AND the spec review runs all four over thousands of generated terms, requiring **0** exceptions before they ship — any failure corrects or drops the law.
...
- **Confluence is vacuous if the generator only emits normal forms** (no redexes → `whnf` = `nf` = `t` trivially). Mitigation: spec review checks whether `Term.typed_term` actually produces reducible terms; if not, the assay's generator branch must seed a redex (e.g. an applied lambda) or the vacuity is surfaced in a health line rather than passing silently.
