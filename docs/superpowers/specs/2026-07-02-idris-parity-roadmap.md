...
| 3 | Pattern matching depth | Nested/deep patterns → decision-tree compiler (M8.4). Kernel case already nests, so this is a lowering pass, not a kernel change. **Single-nested-column lowering landed** (oracle `match/mt07_nested_patterns`, accept/accept): `elaborate_match`'s `desugar_nested_arms` groups arms by outer constructor and, for a group with at most ONE nested argument column (others variables, substituted by their fresh binder), emits `C(v₁..v_k) -> match v_j <inner arms>` — nested *single-level* matches, so motives/index refinement/catch-all/coverage all apply per level and the kernel's own nesting `:case` does the rest. Deeper nesting is lowered on re-entry; a variable sub-pattern in the nested column becomes an inner catch-all. Verified on the BEAM. No TCB. *Reach*: **multi-column** nesting (`MkP(Z(),Z())` — 2 active columns) and a top-level catch-all mixed with nesting are rejected cleanly (`:multi_column_nesting` / `:catchall_with_nesting`) — real multi-column (matrix) compilation is future work | E | additive (refactors match path) | 🟡 (single-column; reach: multi-column) |
