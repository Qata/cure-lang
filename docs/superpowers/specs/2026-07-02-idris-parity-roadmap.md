...
| 3 | Pattern matching depth | Nested/deep patterns → decision-tree compiler (M8.4). Kernel case already nests, so this is a lowering pass, not a kernel change. **Full pattern-matrix lowering landed** (oracle `match/mt07_nested_patterns` + `match/mt08_multicolumn_patterns`, both accept/accept): `elaborate_match`'s `desugar_nested_arms` groups arms by outer constructor and compiles each group's argument columns with the standard matrix algorithm (Augustsson/Maranget) — `compile_matrix`/`compile_matrix_split` do left-to-right column selection, emitting a tree of single-scrutinee matches (all-variable column → drop + substitute; constructor column → branch per ctor with the sub-columns spliced in, plus a catch-all for the variable rows). Every emitted node is single-level, so motives/index refinement/catch-all/coverage all apply per level and the kernel's own nesting `:case` does the rest. Handles arbitrary depth AND multiple nested columns; deeper nesting lowers on re-entry; verified on the BEAM (multi-column runtime). Shadow-guarded (`:shadowed_nested`), no TCB. *Reach*: a top-level catch-all mixed with nesting (`:catchall_with_nesting` — the outer scrutinee column would need matrix treatment too) | E | additive (refactors match path) | 🟡 (depth + multi-column; reach: outer catch-all) |
...
*partially* landed: #3 single-column nested-pattern lowering, #4 var/wildcard
catch-all, #6 capabilities A + B + sibling, #26 checked mode),
