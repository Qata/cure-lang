...
On a proven-exhaustive group, the emitted Core changes in exactly one way: the last guarded arm's boolean-test wrapper is dropped — the previous guard's false-branch becomes that arm's body directly. **The concrete representation differs by trigger site** — implementers must patch each site in its own terms, not search for a shared `bool_case` call:
  - Site `elaborator.ex:2441` (`guard_chain`) constructs Core `bool_case` directly (`{:ok, bool_case(test, expected, tt, ff, ctx)}`); here the wrapper to drop is literally the `bool_case` call.
  - Sites `elaborator.ex:2748` (`build_guard_chain`) and `2987` (`fold_leaf_rows`) instead emit a **surface** `mk_if`/`{:conditional, [], [guard, body, else]}` node, which only becomes `bool_case` later when that surface expression is elaborated through the general `:conditional` clause (`elaborator.ex:957-963`). At these two sites the elision means: for the final row, skip the `mk_if` wrap and emit the row's body directly (the same substitution already applied to every other row via `rename_all`/`subs`), not "call `bool_case` and later remove it."
  In all three cases the context (`names`, `ctx`) and the `expected`/motive type are identical between the guarded and unguarded framing — verified against `guard_chain`'s recursion (every arm, at every depth, is checked against the same `expected` with no additional binder introduced) and against the general `:conditional` clause (both branches checked against the same `expected_core`) — so the elision is well-typed by construction, not merely by Z3's say-so: the emitted term the kernel re-checks is exactly the term it would have checked had the surface guard been an ordinary unguarded catch-all in the same position.
...
- **Unit (new `test/cure/elab/guard_lint_test.exs`):**
  ...
  - Shadowed guard (`x < y` then `x < y` again, catch-all present) → `{:ok, env}` + one `{:guard_shadowed, _, _}` via the §2.5 warnings accessor (not an `Env` field); unshadowed group → no warnings.
