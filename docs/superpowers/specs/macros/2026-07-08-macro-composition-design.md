# Dialect Composition — Stacked Languages, Composed Proofs

**Date:** 2026-07-08
**Status:** design (operator insight, same day). Child of
[`2026-07-08-dialect-facility-design.md`](2026-07-08-dialect-facility-design.md),
extending its §7 (scoping & composition) into a full mechanism. Consumes the
`check` ladder ([`2026-07-08-check-dialect-design.md`](2026-07-08-check-dialect-design.md))
...
generality*: binders, quantifiers, explicit indices. Every dialect trades
...
> **Seam obligations are machine-authored on both sides.** When dialect B is
> nested inside dialect A, the proof obligations arising at the seam are
...
> never arises at all, because in a dialect stack no human writes a type at
> any layer.

This is a closure property, not a coincidence: it holds for every pair of
conforming dialects, at every nesting depth.

**Generality is recovered structurally.** A quantifier is just an outer
dialect. "Stitch counts balance *for all sizes*" is a ∀ written as
...
node/mount/bus declarations. The outer dialect quantifies **by
...
- **`protocol` × `packet` → tag elision.** "At a deterministic step the wire
  ...
  neither dialect can state it alone (protocol has the step-determinism,
...
- **`units` × everything.** The degenerate case: a dialect that exists
  almost entirely as *proof plumbing* — its exported carrier laws are what
  make other dialects' arithmetic obligations well-formed.

What they share: an outer dialect needed a **fact** the inner dialect could
...
A dialect exports not just types but **named, kernel-checked facts**. Two
new member kinds in the `dialect` container:
...
- **Statement form:** a `provides` fact is stated as a `check`-style prop —
  Bool-valued, quantified only over the dialect's declared parameters (`$P`
  ranges over the dialect's own instantiations). Theorem signatures and
...
- **Witness forms:** `proved by construction` (static discharge at each
  ...
  shipped with the dialect's library — the route for *inductive* facts like
...
- **Instantiation:** facts are families. `Packet.roundtrip` is instantiated
  ...
  mechanism, promoted.
- **Resolution:** when an outer dialect's `elab` hits an obligation matching
...
dialect author or library — and the export is the proof.
...
**outer-dialect values**:

```cure
syntax ... $payload:Packet.FieldDecl(max_size: $transport.mtu) ...
```

The inner dialect's grammar/elaboration now receives an index from the outer
dialect's declarations — a fleet transport's MTU constrains the packet
...
the outer dialect's value — and only *dialect authors* ever see it. Users
see: "your frame won't fit ESP-NOW."

Semantics: category parameters are ordinary values of the outer dialect's
quoted-AST/derived types, threaded into the inner dialect's `elab` as extra
...
2. **Fact resolution** — a `requires`-matched, trust-eligible `provides`
   fact (§3).
3. **Failure** — a **composition error**, charged to the dialect authors,
...
> The error names both dialects, states the missing fact in both
...
failure carries both dialects' provenance. Requirements:
...
- Resolution order stays innermost-wins, but a `requires`-declaring dialect
  may register **seam explainers** keyed on (its dialect, embedded dialect,
...
Dialect pairs that intend to compose ship **composition templates** — check
...
dialect text → the surface Cure it expands to → Core**. Every rung shows the
...
- **Seam explosion (N dialects → N² seams?)** — no: seams are demand-driven
...
- **Semantic mismatch that isn't type mismatch** — two dialects with
...
1. **Fact-statement language** — §3 uses check-prop syntax; confirm the
   exact fragment (quantification only over dialect parameters; Bool-valued
...
3. **Fact versioning** — a dialect upgrade that strengthens/weakens a
   provided fact vs. downstream dialects compiled against the old statement;
...
4. **Parameterized-category typing** — the precise rule for which outer
   ...
   how they appear in the inner dialect's quoted-AST types.
5. **Adapter-form convention** (§10) — a facility-level marker so seam
   adapters are recognizable/greppable, or purely per-dialect idiom.
6. **Composition error-code block** — allocate (e.g. E270–E279) in the
   explainer registry's next pass.
7. **Recursive facts** — may a `provides` witness consume another dialect's
...
- **No user-visible proof language.** This spec adds proof *plumbing*
  between dialects; if it ever surfaces a goal to an end user, it has
  failed (§5).
...
- **No cross-dialect semantic unification.** Adapters make model differences
