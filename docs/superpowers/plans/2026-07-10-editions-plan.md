...
- **Create/Modify** tests: `test/cure/edition_test.exs`, `test/cure/project_edition_test.exs`, `test/cure/migrate/{rule_tier,run_to_fixpoint,proto_to_interface,edition_crossing}_test.exs`, `test/cure/compiler/edition_lexer_test.exs`, and an Antigen probe pair.
...
- Produces: `Cure.Edition.pragma_edition/1 :: String.t() | nil`; `Cure.Edition.resolve/1` where the arg is `%{optional(:source) => String.t(), optional(:project_dir) => Path.t()}` returning `{:ok, t()} | {:error, term()}`; `Cure.Project` struct gains `:edition` (a validated edition string or `nil`).
...
```elixir
  ...
  defp resolve_project(dir) do
    case Cure.Project.load(dir) do
      {:ok, %{edition: nil}} -> {:ok, current()}
      {:ok, %{edition: edition}} -> {:ok, edition}
      {:error, :no_project_file} -> {:ok, current()}
      {:error, _} = err -> err
    end
  end
```
...
And lift+validate it in the struct build (the `%__MODULE__{...}` at `lib/cure/project.ex:740`), by inserting an `edition:` entry that validates through `Cure.Edition`. Because the struct is built inside `parse_toml/1` which cannot return `{:error, _}`, thread validation through `load/1` instead: after `project = parse_toml(content)`, validate:

```elixir
  # in load/1, replace the {:ok, ...} arm:
      {:ok, content} ->
        project = parse_toml(content)

        case validate_edition(Map.get(parse_toml_project(content), "edition")) do
          :ok -> {:ok, %{project | root: dir, edition: Map.get(parse_toml_project(content), "edition")}}
          {:error, _} = err -> err
        end
```

To avoid re-parsing, prefer capturing the raw edition string during `parse_toml` and validating in `load/1`. Concretely: in the struct build at `lib/cure/project.ex:740`, set `edition: Map.get(parsed.project, "edition")` (raw string or nil); then in `load/1` validate that value:
...
And add `edition: Map.get(parsed.project, "edition")` to the `%__MODULE__{...}` build at `lib/cure/project.ex:740-742`.
...
```bash
git add lib/cure/migrate/rule.ex lib/cure/migrate.ex test/cure/migrate/rule_tier_test.exs
...
```bash
git add lib/cure/migrate/rules/ test/cure/migrate/rule_tier_test.exs
git commit -m "refactor(migrate): re-tag the five seed rules with tier + provenance"
```
...
In `parse/2` accept `:edition` (`edition = Keyword.get(opts, :edition, Cure.Edition.current())`) and store it on the parser state so any keyword recheck uses it (mirror the lexer). For placement: in the decorator-handling clause (the same area that handles `@group` — around `lib/cure/compiler/parser.ex:4994`), special-case `dec_name == :edition`:

```elixir
      :edition ->
        if file_leading?(state) do
          # a file-leading @edition pragma is a plain decorator node; edition
          # resolution already read it via the pre-parse scan (Cure.Edition).
...
Expected: FAIL — `run_to_fixpoint/2` undefined.
...
```elixir
  ...
  def run_to_fixpoint(ast, opts \\ []) do
    ...
    do_fixpoint(ast, opts, max, [], [])
  end

  defp do_fixpoint(ast, _opts, 0, warns, fired) do
    if fired == [], do: {:ok, ast, warns}, else: {:error, {:no_convergence, Enum.uniq(fired)}}
  end

  defp do_fixpoint(ast, opts, passes_left, warns, _prev_fired) do
    {new_ast, pass_warns} = run(ast, opts)

    if new_ast == ast do
      {:ok, ast, warns ++ pass_warns}
    else
      case verify(new_ast, opts) do
        :ok ->
          fired = Enum.map(pass_warns, & &1.rule)
          do_fixpoint(new_ast, opts, passes_left - 1, warns ++ pass_warns, fired)

        {:error, rule} ->
          {:error, {:verify_failed, rule}}
      end
    end
  end

  # Reprint → reparse; fail if the output does not parse. (Comment-preservation
  # is asserted by the caller-level lossless gate in tests; here we guard the
  # structural reparse, which is the property fixpoint iteration can break.)
  defp verify(ast, _opts) do
    src = Cure.Compiler.Printer.quoted_to_string(ast)

    case Cure.Compiler.Lexer.tokenize(src, emit_events: false) do
      {:ok, toks} ->
        case Cure.Compiler.Parser.parse(toks, emit_events: false) do
          {:ok, _} -> :ok
          {:error, _} -> {:error, :reparse}
        end
...
- Modify: `lib/cure/migrate.ex` (register the rule in `rules/0`)
- Test: `test/cure/migrate/proto_to_interface_test.exs`

**Interfaces:**
- Consumes: the extended `%Rule{}` (Task 3), the AST shapes for `proto`/`impl` (`{:protocol, meta, body}` / `{:implementation, meta, body}` — confirm the exact tags below before writing the rewrite).
- Produces: rule `:W_proto_to_interface`, tier `:machine`, `since: "2026"`, `enforced_in: nil`, `retires_keywords: ["proto", "impl"]`. Rewrites the `proto`/`impl` AST nodes to their `interface`/`implementation` equivalents, preserving bodies and trivia.

- [ ] **Step 1: Confirm the AST tags**

Run: `MIX_ENV=test mix run -e 'src = "mod M\n  proto P(t)\n    fn e(a: t) -> Bool\n  impl P for Int\n    fn e(a: Int) -> Bool = true\n"; {:ok, toks} = Cure.Compiler.Lexer.tokenize(src, emit_events: false); {:ok, ast} = Cure.Compiler.Parser.parse(toks, emit_events: false); IO.inspect(ast, limit: :infinity)' 2>/dev/null`
Expected: reveals the node tags the parser emits for `proto`/`impl` (e.g. `:protocol`/`:trait`/`:implementation`) and for `interface`/`implementation`. Use the printed tags verbatim in Step 3. (The parser routes `:proto` via `parse_protocol` at `lib/cure/compiler/parser.ex:1377`+ and `:interface` via `parse_interface` at `:1386`+; compare their output shapes — the rewrite maps one to the other.)
...
Create `lib/cure/migrate/rules/proto_to_interface.ex`. Using the tags confirmed in Step 1 (shown here as `:protocol`→`:interface` and `:implementation` old→new — substitute the real tags), map the node kind and keep meta/body:
...
defmodule Cure.Migrate.Rules.ProtoToInterface do
  ...
  trivia are preserved because only the node kind (and its rendered keyword)
  changes. `enforced_in: nil` — the keyword stays live until a future edition
  ...
  # Substitute the real legacy/canonical tags confirmed in Step 1.
  defp walk({:protocol, meta, body}, lines) do
    {body, lines} = walk(body, lines)
    {{:interface, meta, body}, [Keyword.get(meta, :line) | lines]}
  end

  defp walk({:impl_block, meta, body}, lines) do
    {body, lines} = walk(body, lines)
    {{:implementation, meta, body}, [Keyword.get(meta, :line) | lines]}
  end
...
If the confirmed AST shape means `proto` and `interface` differ by more than the tag (e.g. a `:kind` field in meta rather than a distinct tag), adjust `walk/2` to rewrite that field instead; the test asserts the rendered output, which is the real contract.
...
- [ ] **Step 5: Monotone + lexer regression**

Run: `MIX_ENV=test mix test test/cure/migrate/monotone_property_test.exs test/cure/compiler/edition_lexer_test.exs test/cure/migrate/`
Expected: PASS — the new rule is idempotent (a file already using `interface`/`implementation` yields `:no_change`), and `retired_keywords("2026")` is still `[]` because this rule's `enforced_in` is `nil`.
...
defmodule Cure.Migrate.EditionCrossingTest do
  ...
  test "rules_for_crossing includes :machine and :review, excludes nothing tier-wise for the current edition" do
    picked = Migrate.rules_for_crossing("2026", Migrate.rules()) |> Enum.map(& &1.id)
    assert :W_module_rename in picked          # :machine
    assert :W_uppercase_type_var in picked     # :review must be included
    refute :W_removed_module in picked         # :manual never auto-applies
  end
...
**Files:**
- Modify: `lib/cure/cli.ex:139` and its `cmd_migrate` implementation
- Test: `test/cure/cli/migrate_edition_cli_test.exs`
...
- Produces: `cure migrate [--edition YYYY] [--check] [--print] [paths…]` that (phase 1) rewrites targets to a fixpoint under the target edition's rule set, then (phase 2) bumps the edition marker — refusing the bump if a `blocking_manual` item remains, and refusing a downgrade target.
...
defmodule Cure.CLI.MigrateEditionCLITest do
  ...
  test "a downgrade target is refused" do
    assert {:error, :downgrade} =
             Cure.CLI.plan_migration(target: "2026", current: "2026", downgrade_probe: "2025")
  end
...
```elixir
  @doc false
  # Pure planning: refuse a downgrade target.
  def plan_migration(opts) do
    target = Keyword.fetch!(opts, :target)
    current = Keyword.get(opts, :current, Cure.Edition.current())
    probe = Keyword.get(opts, :downgrade_probe, current)
    if Cure.Edition.compare(target, probe) == :lt, do: {:error, :downgrade}, else: {:ok, target}
  end
...
Then in `cmd_migrate`: resolve `--edition` (default `Cure.Edition.current()`), call `plan_migration/1` (abort on `:downgrade`), and for each target file call `plan_migration_source/2`; on `{:ok, out, _, bump}` (and not `--check`/`--print`) write the file (after `Cure.Migrate.git_guard/1`) and, when every file succeeded and none was `:blocked`, `Cure.Project.set_edition/2` (whole-project) or splice the `@edition` pragma (single file). On any `:blocked`, print the hand-port sites and skip the bump. `--check` lists would-change files + pending bump and exits non-zero; `--print` writes the migrated source and pending bump to stdout.
...
- [ ] **Step 5: CLI regression**

Run: `MIX_ENV=test mix test test/cure/cli/ test/mix/tasks/cure_rewrite_test.exs`
Expected: PASS — the pre-existing `cure migrate` file-mode behavior (in-place/`--check`/`--print`, git-guard) is preserved; the edition bump is additive.
...
**Placeholder scan:** Task 9 Step 1 legitimately *discovers* AST tags at implementation time (the parser's exact `proto`/`interface` node shape is not knowable from the spec) — this is a real verification step with an exact command and expected output, not a placeholder; Step 3 shows the full rewrite against representative tags and states precisely how to substitute the confirmed ones. No `TBD`/`handle edge cases`/`similar to` remain.
