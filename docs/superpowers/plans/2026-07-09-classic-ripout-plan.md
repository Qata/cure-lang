...
- Maintain a running **LEDGER** in the final report: every deleted test file with count + reason class; every EXTENSION; every ledgered edit; the stdlib disposition table; the examples disposition table.
...
(If the fsm body above fails to PARSE rather than elaborate, adjust the container body to the minimal form the parser accepts — the pinned behavior is the `{:error, …unsupported_container…}` shape from elaboration, and the assertion on `inspect(reason)` tolerates wrapping. If `Cure.Compiler.compile_and_load`'s error wrapping makes even that unreachable, pin via `Cure.Elab.Program.elaborate/1` instead and note it in the ledger.)
