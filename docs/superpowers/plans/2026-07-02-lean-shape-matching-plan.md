...
- [~] **Step 3: implement E-only.** (3a) DONE; (3b) NEXT. Design refined during Steps 1-2 into TWO coordinated sub-parts (both E-only, no `lib/cure/core/*` edit):
  ...
  - **(3b) Sibling refinement over an indexed scrutinee — the genuine carried eq. NEXT.** Red test captured & confirmed RED (commit dbb1fe5, `carried_index_sibling_test.exs`, `@tag :skip`): a sibling `w : F(app(p,q))` returned in a branch whose 3a-refined goal is `F(SNil())` fails `:branch_type` — it needs the branch's stuck equation `app(p,q) = SNil()` carried and `w` transported. **Path finding (this pass):** the `match` path (`elaborate_match`/`build_motive`/`elaborate_branches`) has 3a's goal refinement but NO sibling transport; capability-B transport (`elaborate_with_value` :537) is restricted to NON-indexed families (:543/:579); `elaborate_with_rematch` refines siblings only by index-*variable* substitution (`specialize_branch_context`) — which cannot help a *stuck computed* index (no variable to bind). So 3b is a NEW combination in the match path: generalize the motive over the computed index (3a, done) AND wrap it in an **index-level** Eq-arrow — `eq_arrow_motive` (:758) generalized from scrutinee VALUE `e` to the computed index TERM `app(p,q)`: `motive = λ(j₀).λ(x:F j₀). Eq(SList, app(p,q), j₀) -> G[app(p,q)↦j₀]`, whole case applied to `(refl app(p,q))`; each branch gets `prf : Eq(SList, app(p,q), ctor_idx)` and transports each index-mentioning sibling via `rewrite prf (λx. H_j[app(p,q)↦x]) h_j` (reuse `collect_with_siblings` :770 keyed on the index term; indexed data stays a `:rewrite` motive RESULT → avoids Phase-5 reify collapse). Reuse `check_motive_wf`'s acceptance of `Π(Eq(ty,a,b),G)` (`kernel.ex:597-689`). **NOT on the current FRP-probe critical path** (`frp01`/`frp02` use explicit `rewrite ... in`, never match on SF) — so 3b is spec-generality, not FRP-blocking; when it lands, add an oracle probe that matches SF and needs sibling transport (Idris-accepted) to prove the generality. Unskip the two sibling tests as the red→green driver.
