...
- [ ] **Step 3: implement E-only.** **NEXT.** Design refined during Steps 1-2 into TWO coordinated sub-parts (both E-only, no `lib/cure/core/*` edit):
  - **(3a) Computed-index goal generalization — needs NO carried eq.** `build_motive` (`elaborator.ex:913`) generalizes only index *variables* (`rebind`/`generalize` :925-959 rewrite `{:var,i}` occurrences), SKIPPING a computed index like `app(p,q)` (:930). Extend it to abstract the whole computed-index *occurrence* in the goal (term-occurrence abstraction, like capability-A's `abstract_term` :318, keyed on the index TERM shifted into each binder frame) → each branch's RETURN goal refines to the ctor's result index (`F(app(as,bs))`, `F(SNil())`). This is the standard casesOn motive extended to computed occurrences — sound with no equation (kernel checks the branch at `motive @ s_values`). **The isolated red test (`carried_index_eq_test.exs` test 1, `leaf()->leaf(); mk(l,r)->mk(l,r)`) targets exactly this**; today `{:error,:branch_type}`, green when the goal refines. Soundness-control arm (`mk(r,l)`) stays `{:error,_}`. CAVEAT: occurrence abstraction is sound but *incomplete* — if the goal mentions the index in a different reduced form the occurrence is missed (branch fails, never unsound); acceptable for the FRP cluster where the goal mentions the index literally.
  - **(3b) Sibling refinement over an indexed scrutinee — the genuine carried eq.** A sibling `w : G(app(p,q))` CANNOT be abstracted into the scrutinee's motive (the `Π(indexed…)` reify-collapse capability B deliberately avoids, :491-494). Carry `Eq(IdxTy, orig_idx, idx_binder)` (Eq-arrow motive, capability-B `eq_arrow_motive` :748 generalized value→index) + transport each index-mentioning sibling via `rewrite prf (λx. H_j[idx↦x]) h_j` (:496 — indexed data stays a `:rewrite` motive RESULT, `Eval.apply`'d never reified → avoids Phase-5 collapse). Reuse `check_motive_wf`'s acceptance of `Π(Eq(ty,a,b),G)` (`kernel.ex:597-689`, `defc6cb`). A separate red arm (returning `v`/a sibling in a refined branch) must be added when (3b) is implemented. Attempt (3a) first (contained), then (3b).
