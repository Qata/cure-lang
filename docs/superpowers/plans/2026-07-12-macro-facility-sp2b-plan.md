...
- `MacroValidate` (SP2 slice 1) already has `check_explain_exhaustive/1`; add `check_rules_pinned/1` alongside. `errors.ex` renders via `format_error/2` → `format_diagnostic/5`; place the `:rule_unpinned` clause before the catch-all (`errors.ex:398`), any private helper AFTER it (clause-grouping lesson from SP1 §2).
