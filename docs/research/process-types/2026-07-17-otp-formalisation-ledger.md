...
- [ ] BRec **adequacy** proper — compose `brec_handles` with a recursive `Runs`/`preservation_at` chain (every config reachable by running a recursive behaviour is well-typed at `rec_infer`). Coverage is the fundamental-theorem half; this is the operational-closure half.
