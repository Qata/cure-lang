defmodule Cure.Stdlib.Preload do
  ...
  def builtin_fixity_table do
    with {:ok, path} <- operators_source_path(),
      ...
      build_fixity_table(ast)
    else
      _ -> Cure.Compiler.Parser.FixityTable.new()
    end
  end
  ...
  # operator lexeme.
  defp build_fixity_table(ast) do
    nodes = collect_fixity_nodes(ast)

    table =
      Enum.reduce(nodes, Cure.Compiler.Parser.FixityTable.new(), fn
        {:precedencegroup, meta, _}, acc ->
          Cure.Compiler.Parser.FixityTable.add_group(acc, Keyword.fetch!(meta, :name),
  ...
  defp add_fixity_op(table, meta) do
    ...
    if is_binary(lexeme) and is_atom(group) and not is_nil(group) do
      ...
      case Keyword.get(meta, :fixity) do
        :infix -> Cure.Compiler.Parser.FixityTable.add_infix(table, lexeme, group, opts)
        :prefix -> Cure.Compiler.Parser.FixityTable.add_prefix(table, lexeme, group, opts)
        :postfix -> Cure.Compiler.Parser.FixityTable.add_postfix(table, lexeme, group, opts)
        _ -> table
      end
    else
      table
