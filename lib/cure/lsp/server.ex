defmodule Cure.LSP.Server do
  ...
  defp keyword_completions do
    keywords =
      ~w(fn mod rec actor fsm sup app proto impl type let have if then else elif match return throw try catch finally use local when requires where)
  ...
  def compute_semantic_tokens(text) do
    keywords =
      ~w(fn mod rec fsm proto impl type let have if then else elif match return throw try catch finally use local when requires where)
