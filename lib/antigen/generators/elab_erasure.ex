defmodule Antigen.Generators.ElabErasure do
  ...
  # -- Two-sided catalog: {id, expect, note, fn-body} --------------------------
  @catalog [
    {"type_position", :accept, "erased `n` used only in a type/index position",
     """
    ...
    {"returned", :reject, "erased `n` returned as the value",
     ...
     """},
    {"present_arg", :reject, "erased `n` passed in a present argument position",
     """
       fn g(m: Nat) -> Nat = m
       fn f({n: Nat}, v: NV(n)) -> Nat = g(n)
     """},
    {"scrutinised", :reject, "erased `n` scrutinised as a case discriminant",
     """
  ...
  def erasure_challenges do
    Enum.map(@catalog, fn {id, expect, note, body} ->
      Challenge.new(
        kind: :elab_program,
        assay: "elab/erasure",
        label: expect,
  ...
  def catalog, do: Enum.map(@catalog, fn {id, expect, _n, _b} -> {id, expect} end)
  ...
  def source(id) do
    case Enum.find(@catalog, fn {i, _, _, _} -> i == id end) do
      {_id, _expect, _note, body} -> module(body)
      nil -> nil
    end
  end
  ...
  def metamorphic_challenges do
    Enum.flat_map(@catalog, fn {id, expect, _note, body} ->
