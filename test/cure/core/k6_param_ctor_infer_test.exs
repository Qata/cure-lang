defmodule Cure.Core.K6ParamCtorInferTest do
  ...
  checking mode). `refl : {w:a} -> Eq(a, w, w)` has family parameter `a`; supplied
  ...
  test "infer accepts a param constructor with the param in the spine" do
    ...
    # refl at a=Int, w=3  ->  Eq(Int, 3, 3).  Spine = [param a, witness w].
    term = {:ctor, :refl, [{:int_type}, {:int_lit, 3}]}

    assert {:ok, {:vdata, :Eq, [{:vint_type}, {:vint, 3}, {:vint, 3}]}} =
  ...
  test "infer still rejects a param constructor with params ABSENT (checking-mode only)" do
    ...
    term = {:ctor, :refl, [{:int_lit, 3}]}
    assert {:error, {:ctor_requires_checking_mode, :Eq}} = Kernel.infer(ctx, term)
  end
end
