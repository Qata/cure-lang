defmodule Cure.Core.FloatPrimTest do
  ...
  test "eval folds float comparisons to booleans" do
    assert Eval.eval({:prim, :lt, [{:float_lit, 1.0}, {:float_lit, 2.0}]}, []) == {:vbool, true}
  end
  ...
  test "kernel types Float literals and arithmetic" do
    ...
    assert {:ok, {:vbool_type}} = Kernel.infer(ctx, {:prim, :lt, [{:float_lit, 1.0}, {:float_lit, 2.0}]})
