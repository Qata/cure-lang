defmodule :cure_std_regex_test do
  ...
  #   compile -> Result(Regex, RegexError) -> {:Ok, regex} / {:Error, {:InvalidPattern, msg}}
  #   run     -> Option(Matched)           -> {:Some, {:Matched, whole, groups}} / :None
  # The compiled Regex is opaque (Cure never matches it) and stays a struct map.
  ...
  describe "compile/1 and compile_bang/1" do
    ...
    test "compile returns InvalidPattern for a bad regex" do
      assert {:Error, {:InvalidPattern, _msg}} = :cure_std_regex.compile("a(")
    end

    test "compile returns InvalidPattern for non-binary input" do
      assert {:Error, {:InvalidPattern, _}} = :cure_std_regex.compile(42)
    end
  ...
  describe "run/2" do
    test "returns the first match with capture groups" do
      ...
      assert {:Some, {:Matched, "hello world", ["hello", "world"]}} =
               :cure_std_regex.run(r, "hello world, again")
    end

    test "returns the nullary None() -- the bare atom :None -- when nothing matches" do
  ...
  describe "scan/2" do
    test "returns every match in source order" do
      ...
      assert [{:Matched, "1", _}, {:Matched, "22", _}, {:Matched, "333", _}] = matches
    end
