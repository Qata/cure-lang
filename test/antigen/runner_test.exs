defmodule Antigen.RunnerTest do
  ...
  test "default_gen has exactly 15 branches in the documented group order (guard)" do
    {:frequency, ws} = Mix.Tasks.Antigen.default_gen()
    assert length(ws) == 15
    # position 15 is the structure-directed Primitive generator — a :typed_term
    # producer, so it joins the term-vertical group `t` for edge reweighting.
    assert Antigen.Runner.gen_group_table() ==
             %{f: [1, 2, 3], t: [4, 5, 6, 9, 10, 11, 12, 13, 14, 15], m: [7, 8]}
  end
